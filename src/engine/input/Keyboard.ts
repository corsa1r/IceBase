...
export default class Keyboard extends InputSwitch {

    private broadcastTo: Container;

    constructor() {
        super(Keyboard.KeyMap);
        this.attachEvents(['keydown', 'keyup']);
    }

    protected input(event: iInternalEvent) {
        let eventName = event.state ? Keyboard.EVENTS.KEYDOWN : Keyboard.EVENTS.KEYUP;
    ...
    public broadcast(to: Container) {
        if (!this.broadcastTo) {
            this.broadcastTo = to;
        }
    }

    private attachEvents(events: Array<string>) {
        events.forEach((event: string, index: number) => {
            window.addEventListener(event, (rawEvent: KeyboardEvent) => {
                this.proceed(rawEvent.keyCode, !index);
            });
        });
    }
